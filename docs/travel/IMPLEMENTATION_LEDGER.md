...
| requirement_id | description | status | implementation_files | tests | validation_command | validation_result | limitations |
|---|---|---|---|---|---|---|---|
| TR-400 | Implement vertical-aware provider protocol, descriptors, registry, health and fail-closed credential/approval gates. | IN_PROGRESS | `backend/travel/providers` | `test_amadeus_provider.py`; provider-health integration pending | `python -m pytest -q tests/travel/test_amadeus_provider.py` | Protocol/descriptors/registry and config gates pass; shared health service pending | Providers may implement one vertical. |
...
| TR-406 | Implement Amadeus OAuth with fixed test/production endpoints, token cache and server-only secrets. | IN_PROGRESS | `backend/travel/providers/amadeus.py`; Redis cache integration pending | OAuth/cache/401/config tests | `python -m pytest -q tests/travel/test_amadeus_provider.py` | Fixed origins, client-credentials token cache and approval gate pass | Shared Redis OAuth cache and credentialed smoke are pending. |
