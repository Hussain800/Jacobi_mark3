...
|---|---|---|---|---|---|
| SEC-TR-000 | Resolved (was High; potentially Critical on Linux) | Static-file route | Phase 0 reproduced an out-of-root read. The route now resolves both root and candidate, enforces `commonpath` containment, and has a percent-encoded parent-segment regression. | The known anonymous file-read path now returns 404 while legitimate in-root assets remain available. | Closed by `backend/tests/travel/test_static_asset_containment.py`; retain filesystem least privilege as defense in depth. |
