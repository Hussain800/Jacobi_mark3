...
export default function JacobiTerminal() {
  ...
  const run = useCallback(async () => {
    setRunning(true); setError(null); setShowSwarm(true);
    if (useCache) {
      await new Promise((r) => setTimeout(r, 800));
      setReport(DEMO); setRunning(false); return;
    }
    try {
      ...
      pollRef.current = setInterval(async () => {
        ...
        try {
          ...
          setError(e.message); if (pollRef.current) clearInterval(pollRef.current); setRunning(false);
        }
      }, 1000);
      setTimeout(() => { if (pollRef.current) { clearInterval(pollRef.current); setRunning(false); setError("Probe timed out"); } }, 60000);
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans antialiased selection:bg-white/10">
      ...
      {/* ─── TOP BAR ─────────────────────────────────────────────────── */}
      <header className="h-12 border-b border-white/[0.06] flex items-center px-6 bg-black/80 backdrop-blur-md fixed top-0 left-0 right-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] text-white/30 font-mono">/ adversarial pricing probe</span>
        </div>
        <div className="ml-auto flex items-center gap-4 text-[11px] text-white/30 font-mono">
      ...
      <div className="flex pt-12 min-h-screen">
        {/* ─── SIDEBAR ────────────────────────────────────────────────── */}
        <aside className="w-72 border-r border-white/[0.06] shrink-0 bg-black/40 p-5 flex flex-col gap-5">
          <div className="space-y-3">
            <label className="text-[10px] font-mono text-white/30 uppercase tracking-[0.15em]">Target URL</label>
            <input
              value={url} onChange={(e) => setUrl(e.target.value)}
              disabled={running}
              className="w-full bg-transparent border-b border-white/10 py-2 text-sm text-white/80 font-mono placeholder-white/20 outline-none focus:border-white/30 transition-colors disabled:opacity-40"
              placeholder="https://..."
            />
          </div>
          <div className="space-y-3">
            ...
            <div className="space-y-1">
              {SAMPLE_TARGETS.map((s) => (
                <button
                  ...
                  disabled={running}
                  className="w-full text-left px-3 py-2 rounded text-[11px] font-mono text-white/40 hover:text-white/70 hover:bg-white/[0.03] border border-transparent hover:border-white/[0.06] transition-all truncate disabled:opacity-40"
                >
                  <ChevronRight className="w-2.5 h-2.5 inline mr-2 opacity-30" />
                  {s.label}
                </button>
              ))}
            </div>
          ...
          <label className="flex items-center gap-3 py-2 cursor-pointer group">
            <div className={`w-9 h-5 rounded-full border transition-colors relative ${useCache ? "bg-white border-white" : "bg-transparent border-white/20"}`}>
          ...
          >
            {running ? (
              ...
              <><Zap className="w-3.5 h-3.5" /> RUN MATRIX TOPOLOGY PROBE</>
            )}
          </button>
        ...
        <main className="flex-1 p-6 space-y-6 overflow-y-auto max-h-screen">
          ...
          {!report && !running && !error && (
            <div className="flex flex-col items-center justify-center h-[70vh] text-center">
              ...
              <p className="text-[11px] text-white/10 font-mono mt-2 max-w-md">
                Configure target parameters in the sidebar and execute a matrix scan.
              </p>
            </div>
          )}
            ...
            <div className="flex flex-col items-center justify-center h-[70vh]">
              ...
              <div className="grid grid-cols-6 gap-2 max-w-2xl">
                {Array.from({ length: 24 }).map((_, i) => (
                  <div key={i} className="aspect-square rounded border border-white/[0.06] bg-white/[0.02] flex items-center justify-center">
                    <div className="w-2 h-2 rounded-full bg-white/20 animate-pulse" style={{ animationDelay: `${i * 80}ms` }} />
                  </div>
                ))}
              </div>
            </div>
          ...
          {report && (
            <>
              {/* ── Metric Strip ── */}
              <div className="grid grid-cols-4 gap-px bg-white/[0.06] rounded overflow-hidden">
                {[
                  ...
                  <div key={m.label} className="bg-black p-5">
              ...
              <section>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-[10px] font-mono text-white/30 uppercase tracking-[0.15em]">Agent Swarm &mdash; {report.total_agents} Probes</h3>
                ...
                <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-[1px] bg-white/[0.04]">
                  {report.agents.map((a) => (
                    <div
                      key={a.agent_id}
                      className={`bg-black p-2.5 ${a.status === "success" ? "hover:bg-white/[0.02]" : ""} transition-colors`}
                      title={`${a.agent_id}: ${a.status}${a.price !== null ? ` $${a.price}` : ""}`}
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[9px] font-mono text-white/20">{a.agent_id.replace("AGENT_", "A")}</span>
                        {a.status === "success" && a.price !== null ? (
                          <span className="text-[11px] font-mono text-white/80">${a.price}</span>
                        ) : a.status === "detected" ? (
                          <span className="text-[9px] text-red-400/60 font-mono">BLOCKED</span>
                        ) : (
                      ...
                      {a.response_time_ms && (
                        <p className="text-[8px] font-mono text-white/10 mt-1">{a.response_time_ms}ms</p>
                      )}
                    </div>
                  ))}
                </div>
                      ...
                      return (
                        <div key={g.variable_name} className="flex items-center bg-white/[0.02] px-4 py-3">
                          ...
                          <div className="flex-1 grid grid-cols-3 gap-2 text-[11px] font-mono">
                            ...
                            <span className={`text-center ${sig ? (high ? "text-white" : "text-white/60") : "text-white/20"}`}>
                              {sig ? fmtDelta(g.delta) : "—"}
                            </span>
                            <span className="text-right text-white/20">{g.state_low}</span>
                          </div>
                          {sig && (
                            <div className={`ml-3 px-2 py-0.5 rounded text-[9px] font-mono ${high ? "bg-white/10 text-white/80" : "bg-white/5 text-white/40"}`}>
              ...
              <section>
                ...
                <div className="bg-white/[0.02] border border-white/[0.06] rounded p-4 font-mono text-[11px] leading-relaxed space-y-2">
                  ...
                  {report.gradients.filter(g => g.significant).map(g => (
                    <p key={g.variable_name} className="text-white/30">
                      &nbsp;&nbsp;├─ {g.variable_name}: &Delta;={fmtDelta(g.delta)} ({g.delta_pct.toFixed(1)}%) &middot; t={g.t_statistic.toFixed(2)} &middot; significant
                    </p>
                  ))}
                  {report.gradients.filter(g => !g.significant).map(g => (
                    <p key={g.variable_name} className="text-white/10">
                      &nbsp;&nbsp;└─ {g.variable_name}: &Delta;={fmtDelta(g.delta)} &middot; not significant
                    </p>
                  ))}
              ...
              {/* ── Execution footer ── */}
              <div className="flex items-center justify-between text-[9px] font-mono text-white/10 border-t border-white/[0.04] pt-4">
