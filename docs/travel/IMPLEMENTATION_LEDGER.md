...
| TR-004 | Open a stacked draft PR against `pivot/price-optimization-mvp`. | NOT_STARTED | GitHub draft PR | `gh pr view` | `gh pr view --json ...` | Pending audit commit and push | Must remain draft; do not merge PR #45 to start. |
...
| TR-401 | Implement real search lifecycle accepted→parsed→cache_checked→running→partial→verifying→terminal. | NOT_STARTED | target orchestrator/state/events | state/integration tests | pytest | Pending | Progress must correspond to real work. |
| TR-402 | Implement Redis queue, leases, retries, deadlines, cancellation, rate limits, cache, event replay, OAuth cache and heartbeat. | NOT_STARTED | target redis/jobs/worker | restart/expiry/idempotency tests | pytest + Compose | Pending | Redis outage fails closed for new distributed jobs. |
...
| TR-405 | Stream named SSE events with IDs, heartbeats, replay and `Last-Event-ID`. | NOT_STARTED | target events/API | SSE reconnect tests | pytest | Pending | Disconnect does not silently lose results. |
