...
import {
  Play, Square, Terminal, Activity, DollarSign, TrendingUp,
  Shield, Globe, Smartphone, Cookie, ExternalLink, Loader2,
  AlertTriangle, BarChart3, Network, FileText, X,
  ChevronRight, Zap, Target, Disc, Maximize2, Minimize2,
  ChevronDown, ChevronUp, AlertOctagon, Search, Code,
} from "lucide-react";
import {
...
function clsColor(cls: string): string { switch (cls) {
  case "uniform": return "text-blue-400"; case "selective": return "text-yellow-400";
  case "progressive": return "text-orange-400"; case "aggressive": return "text-red-400";
...
function parseVarCombo(label: string): string {
  const parts = label.split("  ").filter(Boolean);
  return parts.length >= 3 ? parts.slice(1).join(" | ").replace(/_/g, " ") : label;
}

/* ── Mock DOM Snippets for Differential Explorer ─────────────────────── */

const MOCK_DOM_SNIPPETS: Record<string, { text: string; price: string; node: string }> = {
  "AGENT_00": { text: "UA123 JFK→SFO  Economy  $347.00  Nonstop  6h 22m  Depart 8:00AM  Arrive 10:22AM  Seat 14A  Fare Class K  Fully refundable  Includes 1 carry-on  Includes 1 checked bag    Seat selection available  Change fee $0  United Airlines  United   Flight UA123  Airbus A320  On time   Gate B12  Boarding 7:30AM   Basic economy rules apply  No same-day change  No upgrade eligible", price: "$347.00", node: "  <span class=\"fare-amount\">$347.00</span>" },
  "AGENT_01": { text: "UA123 JFK→SFO  Economy  $371.00  Nonstop  6h 22m  Depart 8:00AM  Arrive 10:22AM  Seat 14A  Fare Class L  Fully refundable  Includes 1 carry-on  Includes 1 checked bag    Seat selection $15  Change fee $50  United Airlines  United   Flight UA123  Airbus A320  On time   Gate B12  Boarding 7:30AM   Basic economy rules apply  No same-day change  No upgrade eligible", price: "$371.00", node: "  <span class=\"fare-amount\">$371.00</span>" },
  "AGENT_06": { text: "UA123 JFK→SFO  Economy  $375.00  Nonstop  6h 22m  Depart 8:00AM  Arrive 10:22AM  Seat 14A  Fare Class L  Fully refundable  Includes 1 carry-on  Includes 1 checked bag    Seat selection $15  Change fee $50  United Airlines  United   Flight UA123  Airbus A320  On time   Gate B12  Boarding 7:30AM   Basic economy rules apply  No same-day change  No upgrade eligible", price: "$375.00", node: "  <span class=\"fare-amount\">$375.00</span>" },
  "AGENT_14": { text: "UA123 JFK→SFO  Economy  $361.00  Nonstop  6h 22m  Depart 8:00AM  Arrive 10:22AM  Seat 14A  Fare Class L  Fully refundable  Includes 1 carry-on  Includes 1 checked bag    Seat selection $15  Change fee $50  United Airlines  United   Flight UA123  Airbus A320  On time   Gate B12  Boarding 7:30AM   Basic economy rules apply  No same-day change  No upgrade eligible", price: "$361.00", node: "  <span class=\"fare-amount\">$361.00</span>" },
  "AGENT_18": { text: "UA123 JFK→SFO  Economy  $380.00  Nonstop  6h 22m  Depart 8:00AM  Arrive 10:22AM  Seat 14A  Fare Class M  Fully refundable  Includes 1 carry-on  Includes 1 checked bag    Seat selection $25  Change fee $75  United Airlines  United   Flight UA123  Airbus A320  On time   Gate B12  Boarding 7:30AM   Basic economy rules apply  No same-day change  No upgrade eligible", price: "$380.00", node: "  <span class=\"fare-amount\">$380.00</span>" },
  "AGENT_19": { text: "UA123 JFK→SFO  Economy  $320.00  Nonstop  6h 22m  Depart 8:00AM  Arrive 10:22AM  Seat 14A  Fare Class K  Fully refundable  Includes 1 carry-on  Includes 1 checked bag    Seat selection free  Change fee $0  United Airlines  United   Flight UA123  Airbus A320  On time   Gate B12  Boarding 7:30AM   Basic economy rules apply  No same-day change  No upgrade eligible", price: "$320.00", node: "  <span class=\"fare-amount\">$320.00</span>" },
};

/* ── Mock Network Stream Logs ────────────────────────────────────────── */

const MCP_LOG_TEMPLATES = [
  ...
  (id: string) => `${id}: Target page resolving — DNS lookup via BrightData resolver`,
...
const VAR_KEYS = ["LOCATION_HIGH", "LOCATION_LOW", "DEVICE_HIGH", "DEVICE_LOW", "COOKIE_HIGH", "COOKIE_LOW", "REFERRER_HIGH", "REFERRER_LOW", "CONTROL"];
...
export default function JacobiTerminal() {
  ...
  useEffect(() => () => { if (pollRef.current) clearInterval(pollRef.current); if (streamRef.current) clearInterval(streamRef.current); if (progressRef.current) clearInterval(progressRef.current); }, []);
  ...
  const run = useCallback(async () => {
    setRunning(true); setError(null); setShowSwarm(true); setShowStream(true);
    setSelectedAgent(null); setStreamLogs([]); setProbeProgress(0);

    // Start network stream
    let logCount = 0;
    const agentIds = Array.from({length: 24}, (_, i) => `AGENT_${String(i).padStart(2, "0")}`);
    let agentIdx = 0;
    streamRef.current = setInterval(() => {
      const aid = agentIds[Math.floor(agentIdx / 4) % 24];
      const tpl = MCP_LOG_TEMPLATES[Math.floor(Math.random() * MCP_LOG_TEMPLATES.length)];
      const ts = new Date().toISOString().slice(11, 23).replace("Z", "");
      setStreamLogs(prev => [...prev.slice(-80), `[${ts}] ${tpl(aid)}`]);
      logCount++; agentIdx++;
    }, 120);

    // Progress animation
    progressRef.current = setInterval(() => {
      setProbeProgress(p => Math.min(p + (Math.random() * 6 + 2), 100));
    }, 300);
    ...
    if (useCache) {
      setTimeout(() => {
        ...
        const finalTs = new Date().toISOString().slice(11, 23).replace("Z", "");
        setStreamLogs(prev => [...prev, `[${finalTs}] Pipeline complete — 22/24 agents succeeded`, `[${finalTs}] Jacobian matrix computed — topology: progressive`]);
        setTimeout(() => { setReport(DEMO); setRunning(false); }, 400);
      }, 2400);
      return;
    }
    try {
      const r1 = await fetch("http://localhost:8000/api/probe", {
      ...
      pollRef.current = setInterval(async () => {
        ...
        } catch (e: any) { setError(e.message); if (pollRef.current) clearInterval(pollRef.current); if (streamRef.current) clearInterval(streamRef.current); if (progressRef.current) clearInterval(progressRef.current); setRunning(false); }
      }, 1000);
  ...
  const agentDOM = selectedAgent ? MOCK_DOM_SNIPPETS[selectedAgent.agent_id] || MOCK_DOM_SNIPPETS["AGENT_00"] : null;
  const baselineDOM = MOCK_DOM_SNIPPETS["AGENT_00"];

  return (
    <div className="min-h-screen bg-black text-white font-sans antialiased selection:bg-white/10">

      {/* ─── TOP BAR ─────────────────────────────────────────────────── */}
      <header className="h-12 border-b border-neutral-900 flex items-center px-6 bg-black/80 backdrop-blur-md fixed top-0 left-0 right-0 z-50">
        <div className="flex items-center gap-3">
          <div className="w-7 h-7 rounded border border-neutral-800 flex items-center justify-center">
            <Network className="w-3.5 h-3.5 text-white/70" />
          </div>
          <span className="text-sm font-medium tracking-tight">JACOBI</span>
          <span className="text-[11px] text-white/20 font-mono">/ adversarial pricing probe</span>
        </div>
        <div className="ml-auto flex items-center gap-4 text-[11px] text-white/20 font-mono">
          <span>{running ? "// SCANNING..." : report ? "// RESULTS READY" : "// STANDBY"}</span>
      ...
      <div className="flex pt-12 min-h-screen">
        ...
        {/* ─── SIDEBAR ──────────────────────────────────────────────── */}
        <aside className="w-72 border-r border-neutral-900 shrink-0 bg-black/40 flex flex-col">
          <div className="p-5 space-y-4 flex-1 overflow-y-auto">
            <div className="space-y-3">
              <label className="text-[10px] font-mono text-white/30 uppercase tracking-[0.15em]">Target URL</label>
              <input value={url} onChange={(e) => setUrl(e.target.value)} disabled={running}
                className="w-full bg-transparent border-b border-neutral-800 py-2 text-sm text-white/80 font-mono placeholder-white/20 outline-none focus:border-white/30 transition-colors disabled:opacity-40" placeholder="https://..." />
            </div>
            <div className="space-y-3">
              <label className="text-[10px] font-mono text-white/30 uppercase tracking-[0.15em]">Target Label</label>
              <input value={name} onChange={(e) => setName(e.target.value)} disabled={running}
                className="w-full bg-transparent border-b border-neutral-800 py-2 text-sm text-white/80 font-mono placeholder-white/20 outline-none focus:border-white/30 transition-colors disabled:opacity-40" />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-mono text-white/30 uppercase tracking-[0.15em]">Quick Select</label>
              <div className="space-y-1">
                {SAMPLE_TARGETS.map((s) => (
                  <button key={s.label} onClick={() => selectSample(s)} disabled={running}
                    className="w-full text-left px-3 py-2 rounded text-[11px] font-mono text-white/40 hover:text-white/70 hover:bg-white/[0.03] border border-transparent hover:border-neutral-800 transition-all truncate disabled:opacity-40">
                    <ChevronRight className="w-2.5 h-2.5 inline mr-2 opacity-30" />{s.label}
                  </button>
                ))}
              </div>
            </div>
            <label className="flex items-center gap-3 py-2 cursor-pointer group">
              <div className={`w-9 h-5 rounded-full border transition-colors relative ${useCache ? "bg-white border-white" : "bg-transparent border-neutral-700"}`}>
                <div className={`w-3.5 h-3.5 rounded-full bg-black absolute top-0.5 transition-transform ${useCache ? "translate-x-[14px]" : "translate-x-[2px]"}`} />
              </div>
              <span className="text-[11px] font-mono text-white/40 group-hover:text-white/60">Demo Resilience</span>
            </label>
            ...
            {error && (
              <div className="flex items-start gap-2 text-[11px] text-red-400/80 font-mono p-3 border border-red-900/30 bg-red-950/20 rounded">
                <AlertTriangle className="w-3 h-3 mt-0.5 shrink-0" />{error}
              </div>
            )}
          </div>

          {/* ─── NETWORK STREAM TERMINAL ────────────────────────────── */}
          <div className="border-t border-neutral-900">
            <button onClick={() => setShowStream(!showStream)}
              className="w-full flex items-center justify-between px-5 py-2.5 text-[10px] font-mono text-white/30 uppercase tracking-[0.15em] hover:text-white/50 transition-colors">
              <span className="flex items-center gap-2"><Terminal className="w-3 h-3" />{running ? "LIVE NETWORK STREAM" : "BRIGHTDATA NETWORK STREAM"}</span>
              {showStream ? <ChevronDown className="w-3 h-3" /> : <ChevronUp className="w-3 h-3" />}
            </button>
            {showStream && (
              <div className="h-36 overflow-y-auto px-5 pb-3 font-mono text-[10px] leading-relaxed space-y-0.5 bg-black/60">
                {streamLogs.length === 0 && !running && (
                  <p className="text-white/10 italic">Awaiting probe execution...</p>
                )}
                {streamLogs.length === 0 && running && (
                  <p className="text-white/20 animate-pulse">Initializing BrightData MCP server...</p>
                )}
                {streamLogs.map((line, i) => (
                  <p key={i} className={
                    ...
                    line.includes("blocked") || line.includes("BLOCKED") ? "text-red-400/60" :
                    line.includes("Proxy routed") ? "text-cyan-400/50" :
                    line.includes("TLS") ? "text-purple-400/50" :
                    line.includes("Price extracted") ? "text-white/80" :
                    "text-white/30"
                  }>{line}</p>
                ))}
        ...
        {/* ─── MAIN PANEL ────────────────────────────────────────────── */}
        <main className="flex-1 p-6 space-y-6 overflow-y-auto max-h-screen">

          {!report && !running && !error && (
            <div className="flex flex-col items-center justify-center h-[70vh] text-center">
              <Disc className="w-12 h-12 text-white/[0.04] mb-6" />
              <p className="text-sm text-white/20 font-mono">Awaiting topology probe</p>
              <p className="text-[11px] text-white/10 font-mono mt-2 max-w-md">Configure target parameters in the sidebar and execute a matrix scan.</p>
            </div>
          )}

          {running && !report && (
            <div className="flex flex-col items-center justify-center h-[70vh]">
              <div className="flex items-center gap-2 text-white/60 font-mono text-sm mb-8">
                <Loader2 className="w-4 h-4 animate-spin" />
                <span className="animate-pulse">Deploying 24 probe agents across 3 staggered waves...</span>
              </div>
              {/* ── RIPPLE ANIMATION GRID ── */}
              <div className="grid grid-cols-6 gap-2 max-w-2xl">
                {Array.from({ length: 24 }).map((_, i) => (
                  <div key={i}
                    className="aspect-square rounded border border-neutral-800 bg-black flex items-center justify-center"
                    style={{ animation: `ripplePulse 2.5s ease-in-out ${i * 80}ms infinite` }}>
              ...
              <style>{`
                @keyframes ripplePulse {
                  0%, 100% { border-color: rgba(255,255,255,0.06); background: rgba(0,0,0,1); }
                  30% { border-color: rgba(255,255,255,0.2); background: rgba(255,255,255,0.03); }
                  60% { border-color: rgba(255,255,255,0.08); background: rgba(0,0,0,1); }
                }
              `}</style>
            </div>
          )}
          ...
          {report && (
            <>
              {/* ── Metric Strip with Crosshair Accents ── */}
              <div className="grid grid-cols-4 gap-px bg-neutral-900 rounded overflow-hidden relative">
                ...
                {[
                  { label: "Baseline Price", value: `$${report.baseline_price?.toFixed(0) ?? "—"}`, sub: `${report.successful_agents} agents`, icon: DollarSign },
                  { label: "Max Spread", value: `$${report.max_price_spread?.toFixed(0) ?? "—"}`, sub: `${report.max_price_spread_pct?.toFixed(1)}% of baseline`, icon: TrendingUp },
                  { label: "Discrimination Index", value: `$${report.discrimination_index.toFixed(0)}`, sub: "cumulative delta", icon: Activity },
                  ...
                  <div key={m.label} className="bg-black p-5 relative">
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-mono text-white/30 mb-3 uppercase tracking-[0.1em]">
              ...
              {report.error && (
                <div className="flex items-start gap-3 p-4 border border-orange-900/30 bg-orange-950/10 rounded text-[12px] font-mono text-orange-400/70">
                  <AlertTriangle className="w-4 h-4 mt-0.5 shrink-0" />{report.error}
                </div>
              )}

              {/* ── Swarm Grid (Clickable) ── */}
              <section>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-[10px] font-mono text-white/30 uppercase tracking-[0.15em]">Agent Swarm &mdash; click agent for DOM exploration</h3>
                  <span className="text-[10px] font-mono text-white/20">{report.elapsed_seconds.toFixed(1)}s &middot; &sigma;={report.control_stability.toFixed(3)}</span>
                </div>
                <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-[1px] bg-neutral-900">
                  {report.agents.map((a) => {
                    const isBlocked = a.bot_detected;
                    const sel = selectedAgent?.agent_id === a.agent_id;
                    return (
                      <button key={a.agent_id} onClick={() => setSelectedAgent(sel ? null : a)}
                        className={`bg-black p-2.5 text-left transition-all ${isBlocked ? "bg-red-950/20" : sel ? "bg-white/[0.04]" : "hover:bg-white/[0.02]"} ${isBlocked ? "animate-blockPulse" : ""}`}
                        title={`${a.agent_id}: ${a.status}${a.price !== null ? ` $${a.price}` : ""}`}>
                        <div className="flex items-center justify-between mb-1.5">
                          <span className={`text-[9px] font-mono ${isBlocked ? "text-red-400/60" : "text-white/20"}`}>{a.agent_id.replace("AGENT_", "A")}</span>
                          {a.status === "success" && a.price !== null ? (
                            <span className="text-[11px] font-mono text-white/80">${a.price}</span>
                          ) : isBlocked ? (
                            <AlertOctagon className="w-3 h-3 text-red-400/60" />
                          ) : (
                            <span className="text-[9px] text-white/20 font-mono">FAIL</span>
                          )}
                        </div>
                        <p className="text-[8px] font-mono text-white/20 leading-tight truncate">{parseVarCombo(a.label)}</p>
              ...
              <style>{`
                ...
                .animate-blockPulse { animation: blockPulse 2s ease-in-out infinite; border: 1px solid rgba(220,38,38,0.1); }
              `}</style>

              {/* ── DIFFERENTIAL DOM EXPLORER ── */}
                ...
                <section className="border border-neutral-900 rounded overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-900 bg-black/60">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-[10px] font-mono text-white/30 uppercase tracking-[0.15em]">Differential DOM Explorer</h3>
                    </div>
                    <div className="flex items-center gap-3 text-[10px] font-mono">
                      <span className="text-white/20">Baseline: <span className="text-white/50">AGENT_00 — $347.00</span></span>
                      <span className="text-white/20">vs</span>
                      <span className="text-white/20">Selected: <span className={selectedAgent.price ? "text-white/80" : "text-red-400/60"}>{selectedAgent.agent_id}{selectedAgent.price !== null ? ` — $${selectedAgent.price}` : " — BLOCKED"}</span></span>
                      <button onClick={() => setSelectedAgent(null)} className="text-white/20 hover:text-white/60 transition-colors"><X className="w-3 h-3" /></button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-px bg-neutral-900">
                    <div className="bg-black p-4">
                      <div className="text-[9px] font-mono text-white/15 mb-2 uppercase tracking-wider">Baseline DOM (AGENT_00)</div>
                      <div className="font-mono text-[10px] text-white/40 leading-relaxed whitespace-pre-wrap break-all">{baselineDOM.text}</div>
                      <div className="mt-3 pt-3 border-t border-neutral-900">
                        <div className="text-[9px] font-mono text-white/15 mb-1">Price node:</div>
                        <code className="text-[10px] text-emerald-400/80 bg-emerald-950/20 px-2 py-1 rounded block">{baselineDOM.node}</code>
                      </div>
                    </div>
                    <div className="bg-black p-4">
                      <div className="text-[9px] font-mono text-white/15 mb-2 uppercase tracking-wider">Selected Agent DOM ({selectedAgent.agent_id})</div>
                      <div className="font-mono text-[10px] leading-relaxed whitespace-pre-wrap break-all">
                        {agentDOM.text.split(" ").map((word, i) => {
                          const isPrice = /^\$\d/.test(word);
                          const isDiffPrice = isPrice && word !== "$347.00";
                          return isDiffPrice ? (
                            <span key={i} className="text-amber-300/90 bg-amber-300/10 px-0.5 rounded">{word} </span>
                          ) : (
                      ...
                      <div className="mt-3 pt-3 border-t border-neutral-900">
                        <div className="text-[9px] font-mono text-white/15 mb-1">Price node:</div>
                        <code className={`text-[10px] ${selectedAgent.price !== null && selectedAgent.price !== 347 ? "text-amber-300/80 bg-amber-950/20" : "text-white/40 bg-white/5"} px-2 py-1 rounded block`}>{agentDOM.node}</code>
                      </div>
                      {selectedAgent.price !== null && selectedAgent.price !== 347 && (
                        <div className="mt-2 flex items-center gap-2 text-[10px] font-mono">
                          <span className="text-white/20">Delta vs baseline:</span>
                          <span className={selectedAgent.price > 347 ? "text-red-400/80" : "text-emerald-400/80"}>
                            {selectedAgent.price > 347 ? "+" : ""}${(selectedAgent.price - 347).toFixed(0)}
                          </span>
                        </div>
                      )}
                      {selectedAgent.bot_detected && (
                        <div className="mt-2 flex items-center gap-2 text-[10px] font-mono text-red-400/60">
                          <AlertOctagon className="w-3 h-3" />BLOCKED — {selectedAgent.detection_signal}
                        </div>
                      )}
                    </div>
                  </div>
                    ...
                    {report.gradients.map((g) => {
                      ...
                      return (
                        <div key={g.variable_name} className="flex items-center bg-white/[0.02] px-4 py-3">
                          <div className="w-32 flex items-center gap-2 text-[11px] font-mono text-white/50">
                            {VAR_ICONS[g.variable_name]}{g.variable_name.replace("_", " ")}
                          </div>
                          ...
                          {sig && (
                            <div className={`ml-3 px-2 py-0.5 rounded text-[9px] font-mono ${high ? "bg-white/10 text-white/80" : "bg-white/5 text-white/40"}`}>{g.delta_pct.toFixed(1)}%</div>
                          )}
                        </div>
                      );
                    })}
                        ...
                        <BarChart data={histData} margin={{ top: 4, right: 8, left: 0, bottom: 16 }}>
                          ...
                          <Bar dataKey="count" radius={[1, 1, 0, 0]}>
                            {histData.map((e, i) => {
                              ...
                              const extreme = pct > 0.08;
                              return <Cell key={i} fill={extreme ? (e.bucket < (report.baseline_price ?? 0) ? "rgba(255,255,255,0.6)" : "rgba(255,255,255,0.8)") : "rgba(255,255,255,0.15)"} />;
                            })}
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
              ...
              <section>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-[10px] font-mono text-white/30 uppercase tracking-[0.15em]">Audit Terminal</h3>
                </div>
                <div className="bg-white/[0.02] border border-neutral-900 rounded p-4 font-mono text-[11px] leading-relaxed space-y-2">
                  ...
                  {report.gradients.filter(g => g.significant).map(g => (
                    <p key={g.variable_name} className="text-white/30">&nbsp;&nbsp;├─ {g.variable_name}: &Delta;={fmtDelta(g.delta)} ({g.delta_pct.toFixed(1)}%) &middot; t={g.t_statistic.toFixed(2)} &middot; significant</p>
                  ))}
                  {report.gradients.filter(g => !g.significant).map(g => (
                    <p key={g.variable_name} className="text-white/10">&nbsp;&nbsp;└─ {g.variable_name}: &Delta;={fmtDelta(g.delta)} &middot; not significant</p>
                  ))}
                  <p className="text-white/20 pt-1">// {report.successful_agents}/{report.total_agents} agents succeeded &middot; DI=${report.discrimination_index.toFixed(0)} &middot; class={report.topology_class}</p>
                </div>
              </section>

              <div className="flex items-center justify-between text-[9px] font-mono text-white/10 border-t border-neutral-900 pt-4">
